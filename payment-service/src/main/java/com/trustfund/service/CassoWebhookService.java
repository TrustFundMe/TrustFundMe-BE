...
public class CassoWebhookService {
    ...
    public void handleCassoWebhook(Map<String, Object> payload, String signature) {
        ...
        for (Map<String, Object> data : dataList) {
            ...
            // Normalize MBB to MB to match system's internal codes
            if ("MBB".equalsIgnoreCase(bankAbbreviation)) {
                bankAbbreviation = "MB";
            }
    ...
    private boolean verifyCassoWebhookKey(String accountNumber, String bankCode, String signature) {
        try {
            ...
            // Fallback for MB/MBB mismatch
            if (bankAccount == null && ("MB".equalsIgnoreCase(bankCode) || "MBB".equalsIgnoreCase(bankCode))) {
                String altCode = "MB".equalsIgnoreCase(bankCode) ? "MBB" : "MB";
                String altUrl = identityServiceUrl + "/api/internal/bank-accounts/by-account-number?accountNumber=" + accountNumber + "&bankCode=" + altCode;
                log.info("Retrying with alternate bank code: {}", altUrl);
                try {
                bankAccount = restTemplate.getForObject(altUrl, Map.class);
                } catch (Exception e) {
                    log.error("Alternate lookup also failed for account {}", accountNumber);
                }
            }
            if (bankAccount == null) {
            ...
            donationRepository.findById(donationId).ifPresent(donation -> {
                if ("PENDING".equals(donation.getStatus())) {
                    ...
                    if (donation.getPayment() != null) {
                        donation.getPayment().setStatus("PAID");
                    }
                    
                    // Update campaign balance
                    donationService.updateCampaignBalance(donation);
                    log.info("Donation {} marked as PAID via Casso", donationId);
                }
            });
        }
    }
    ...
    public List<CassoTransaction> getTransactionsByCampaign(Long campaignId) {
        return cassoTransactionRepository.findByCampaignIdOrderByCreatedAtDesc(campaignId);
    }
