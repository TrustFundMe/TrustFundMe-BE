...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(@NonNull HttpServletRequest request, @NonNull HttpServletResponse response,
        ...
        if (authHeader == null || !authHeader.startsWith("Bearer ")) {
            if (!path.contains("/api/payments/casso") && !path.contains("/api/payments/webhook")) {
        ...
        try {
            ...
            log.info("🔑 [JWT] Token parsed: username={}, role={}, path={}", username, role, path);

            if (username == null || username.isEmpty() || role == null || role.isEmpty()) {
                log.warn("❌ [JWT] Invalid token: missing username or role for path: {}", path);
                filterChain.doFilter(request, response);
                return;
            }
            ...
            log.error("❌ [JWT] Token validation failed for path={}: {} - {}", path, e.getClass().getSimpleName(),
                    e.getMessage());
        }
