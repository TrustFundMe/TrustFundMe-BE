...
public interface ExpenditureItemRepository extends JpaRepository<ExpenditureItem, Long> {
        ...
        @Query(value = "UPDATE expenditure_items " +
                        "SET reservations = 1, updated_at = NOW() " +
                        "WHERE id = :itemId " +
                        "  AND reservations = 0 " +
        ...
        @Modifying
        @Query(value = "UPDATE expenditure_items " +
                        "SET reservations = 0, updated_at = NOW() " +
                        "WHERE id = :itemId " +
                        "  AND reservations = 1", nativeQuery = true)
        int releaseReservation(@Param("itemId") Long itemId);

        List<ExpenditureItem> findByReservationsAndUpdatedAtBefore(Integer reservations, java.time.LocalDateTime time);
}
